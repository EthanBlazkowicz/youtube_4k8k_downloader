...
print("")
console.print("[bold blue]YOUTUBE [green]4K/8K [#EE9427]Downloader")
print("")
    ...
    *list(
        map(
            lambda x: (
                "❌"
                if x["size"] == "0"
                else f'✅ {x["size"]} MB [{", ".join(x["codecs"])}]'
            ),
            RESOLUTIONS,
        )
    )
...
if index is None:
    ...
    index = 1
    PrintWarning("You entered the wrong option, the first item will be downloaded!")
...
if len(available_codecs) > 1:
    ...
    for ci, c in enumerate(available_codecs):
        raw = raw_codecs[ci]
        console.print(
            f"\t[blue]{str(ci+1)}.  [green]{c}[/green] ({codec_sizes.get(raw, '?')} MB)"
        )
...
opener = (
    ...
    else "xdg-open" if sys.platform == "linux" else "start"
)
...
cmd = (" ").join(
    [
        "ffmpeg",
        "-i",
        os.path.join(output_dir, temp_video),
        "-i",
